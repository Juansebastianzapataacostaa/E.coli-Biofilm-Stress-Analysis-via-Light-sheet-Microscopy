...
System setup. In A, we can see an image of the general LSFM-SPIM setup, starting with a 488nm laser passing through an AOTF for modulation. The chip is placed inside a water-filled chamber for sample acquisition through the coverslip. B shows a close-up highlighting the specific sample identification using a dedicated detection objective.
